...
public class CoDevedores2 {
	...
	public static void main(String[] args) throws IOException {
		...
		for(String comp : apontados){
			if(comp.length()>18){
				if(comp.substring(17,18).equals("/") 
						&& comp.substring(20,21).equals("/")
						&& comp.substring(29,30).equals("/")
						&& comp.substring(32,33).equals("/")){
		...
		for(String chave : chaves){
			if(mapa.get(chave)>1){
				cont++;
				saida.write(String.format("%s,CERINFO%3d",chave,cont));
				saida.newLine();
