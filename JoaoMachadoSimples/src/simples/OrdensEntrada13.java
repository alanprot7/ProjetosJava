...
public class OrdensEntrada13 {
	...
	public static void main(String[] args) throws IOException {
		...
		List<AuxArq> porRegistro = new ArrayList<>();
		...
		List<Path> listaDelete = corrigeArquivo(listaArquivos, charset);
		
		for(Path dele : listaDelete){
			Files.deleteIfExists(dele);
		}
		...
		for(int i=0;i<listaArquivos.length;i++){
			...
			if(listaArquivos[i].toString().substring(listaArquivos[i].toString().length()-2,listaArquivos[i].toString().length()-1).equals("7")
				...
				AuxArq registro = new AuxArq();
				registro.setNome(pathApontados.getFileName().toString());
				registro.setQuantidade((cont-2));
				porRegistro.add(registro);
			}
		...
		saida.newLine();
		saida.newLine();
		for(AuxArq aux : porRegistro){
			saida.write(aux.getNome()+" = "+String.format("%3d",aux.getQuantidade()));
			saida.newLine();
		}
	...
	public static List<Path> corrigeArquivo(File[] listaArquivos, Charset charset) throws IOException{
			List<Path> listaDelete = new ArrayList<>();
		
		for(File arq : listaArquivos){
			if(arq.toString().indexOf("D")!=-1 && arq.toString().length()>29){
				int tamIni = 18;
				int tamFim = arq.toString().length()-11;
				String arquivoNovo = arq.toString().substring(0,tamIni)+arq.toString().substring(tamFim,arq.toString().length());
				Path path = Paths.get(arquivoNovo);
				Path pathEn = Paths.get(arq.toString());
				listaDelete.add(pathEn);
				BufferedWriter saida = Files.newBufferedWriter(path, charset);
				List<String> conteuArq = Files.readAllLines(pathEn, charset);
				
				for(String linhas : conteuArq){
					String meio = arquivoNovo.substring(18,21);
					saida.write(linhas.substring(0,1)+meio+linhas.substring(4,linhas.length()));
					saida.newLine();
				}
				saida.flush();
				saida.close();
